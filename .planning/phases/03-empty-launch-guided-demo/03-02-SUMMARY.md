...
6. Created `tests/test_demo_session_isolation.py` verifying the bundled ~10-second A/V asset, package data seam and frozen model lookup, isolated real-controller invocation, persistent profile byte stability, terminal cleanup, idempotency, cleanup protections, normal/demo signal separation, and real `JobController` request construction.
