...
4. Added `Backend.start_demo_job`/`end_demo_job` and a separate real `JobController` configured only against the demo workspace, with JSON-safe operation/session lifecycle events.
5. Created `tests/test_demo_session_isolation.py` verifying the bundled ~10-second A/V asset and package manifest, isolated real-controller invocation, persistent profile byte stability, terminal cleanup, idempotency, and cleanup protections.
