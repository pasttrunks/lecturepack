...
- `RuntimeSetupGateModel` is the sole mutable runtime-gate lifecycle reducer; the DOM controller renders its snapshots, filters stale/terminal events through it, keeps the app inert, and delays normal bridge activity until canonical admission permits it.
...
- Failed events classified offline enter the restricted offline state with an assertive announcement; matching admitted/ready is also assertive.
...
**Authoritative-state correction:** The detached DOM lifecycle fields were removed. `RuntimeSetupGateModel` now owns state, return state, retry/cancel pending flags, active operation, terminal status, and offer data; begin-repair preserves its repairing state rather than resetting it to gate. The executable reducer coverage now exercises bootstrap, all seven states, nonterminal progress/retry/activation, cancellation, retry assessment, diagnostics return, offline/generic failure, matching admission, and stale/duplicate terminal suppression.
...
- Current focused verification passed: `6 passed` for the setup-gate suite.
- Current cross-UI verification passed: `57 passed` across setup-gate, bridge, responsive/motion, and theme suites.
- Current executable reducer sequence covers `gate → confirm → repairing → cancel → gate → offline → failed → diagnostics/back → ready`, bootstrap healthy/setup-required, nonterminal progress/retrying/activated, retry pending/result, and stale/duplicate-terminal suppression.
- Full verification reached `791 passed, 1 failed`; the sole failure is the documented unset `LECTUREPACK_ONEDIR_FIXTURE` prerequisite.
